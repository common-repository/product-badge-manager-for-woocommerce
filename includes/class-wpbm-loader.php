...
class Wpbm_Loader {
    ...
    /**
     * Add a new remove action to the collection to be registered with WordPress.
     *
     * @param    string               $hook             The name of the WordPress action that is being registered.
     * @param    object               $component        A reference to the instance of the object on which the action is defined.
     * @param    string               $callback         The name of the function definition on the $component.
    ...
    public function remove_action( $hook, $component, $callback, $priority = 10, $accepted_args = 1 ) {
        $this->remove_actions = $this->add( $this->remove_actions, $hook, $component, $callback, $priority, $accepted_args );
    }
	...
	public function run() {
		...
		foreach ( $this->actions as $hook ) {
		    if ( $hook['hook'] == 'woocommerce_after_shop_loop'  ) {
		        var_dump( $hook );
		        die();
            }
